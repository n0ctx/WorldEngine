...
test('buildPrompt 组装系统段、历史消息、post prompt 和当前用户消息', async () => {
  ...
  assert.match(result.messages[1].content, /世界知识：群星海/);
  assert.equal(result.messages[2].content, '第一轮提问');
  assert.equal(result.messages[3].content, '第一轮回答');
  assert.match(result.messages[4].content, /第二轮提问/);
  assert.match(result.messages[4].content, /全局后置：阿塔/);
  assert.match(result.messages[4].content, /角色后置/);
});
...
test('buildPrompt 在开启状态栏、召回展开、日记注入与 suggestion 时注入完整矩阵', async () => {
  ...
  assert.equal(result.messages.length, 5);
  assert.equal(result.recallHitCount, 0);
  assert.match(result.messages[1].content, /天气/);
  assert.match(result.messages[1].content, /体力/);
  assert.match(result.messages[1].content, /心情/);
  assert.match(result.messages[1].content, /\[日记注入\]\n昨天的日记摘要/);
  assert.match(result.messages.at(-1).content, /全局后置：阿塔/);
  assert.equal(result.messages[2].content, '旧问题');
  assert.equal(result.messages[3].content, '旧回答');
...
test('buildPrompt 角色 system_prompt 注入 cached system，always 条目注入 dynamic', async () => {
  ...
  assert.match(result.messages[1].content, /后置内容/);
...
test('buildWritingPrompt 在写作模式下合并多角色条目、状态与写作专属开关', async () => {
  ...
  assert.match(result.messages[1].content, /角色一：阿尔法/);
  assert.match(result.messages[1].content, /角色二：贝塔/);
  assert.match(result.messages[1].content, /世界知识：群像世界/);
