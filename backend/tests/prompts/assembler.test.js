...
test('buildPrompt 在开启状态栏、召回展开、日记注入与 suggestion 时注入完整矩阵', async () => {
  ...
  const world = insertWorld(sandbox.db, { name: '矩阵世界', system_prompt: '世界设定：{{world}}' });
  ...
  insertGlobalEntry(sandbox.db, { mode: 'chat', title: '全局条目', content: '全局知识：{{world}}', keywords: ['第二轮'] });
  insertWorldEntry(sandbox.db, world.id, { title: '世界条目', content: '世界知识：{{world}}', keywords: ['第二轮'] });
  insertCharacterEntry(sandbox.db, character.id, { title: '角色条目', content: '角色知识：{{char}}', keywords: ['第二轮'] });
  ...
  assert.match(result.messages.at(-2).content, /全局后置：阿塔/);
...
test('buildPrompt position=post 条目注入到 postParts（messages.at(-2)）', async () => {
  sandbox.writeConfig({
    ...sandbox.readConfig(),
    global_system_prompt: '',
    global_post_prompt: '',
  ...
  const world = insertWorld(sandbox.db, { name: '后置条目世界', system_prompt: '世界系统提示' });
  const character = insertCharacter(sandbox.db, world.id, { name: '测试角色' });
  insertWorldEntry(sandbox.db, world.id, {
    title: '后置条目',
    content: '后置内容',
  ...
  // messages[0] 是 systemParts（含世界系统提示），不应包含 post 条目内容
  assert.doesNotMatch(result.messages[0].content, /后置内容/);
  // postParts 消息在 messages.at(-2)
  assert.match(result.messages.at(-2).content, /后置内容/);
});
...
test('buildPrompt inject_prompt rounds_remaining=0 时不再注入', async () => {
  ...
  assert.doesNotMatch(result.messages.at(-2).content, /触发注入/);
});
...
test('buildWritingPrompt 在写作模式下合并多角色条目、状态与写作专属开关', async () => {
  ...
  const world = insertWorld(sandbox.db, { name: '群像世界', system_prompt: '世界设定：{{world}}', post_prompt: '世界后置' });
  ...
  insertCharacterEntry(sandbox.db, alpha.id, { title: '阿尔法条目', content: '条目命中：{{char}}', keywords: ['当前场景'], position: 'system' });
  insertCharacterEntry(sandbox.db, beta.id, { title: '贝塔条目', content: '条目命中：{{char}}', keywords: ['当前场景'], position: 'system' });
  ...
  assert.match(result.messages[0].content, /条目命中：阿尔法/);
  assert.match(result.messages[0].content, /条目命中：贝塔/);
  assert.match(result.messages.at(-2).content, /写作后置：阿尔法\n\n世界后置/);
