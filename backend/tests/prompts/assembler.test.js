...
test('buildPrompt 在开启状态栏、召回展开、日记注入与 suggestion 时注入完整矩阵', async () => {
  ...
  assert.match(result.messages[3].content, /next_prompt/i);
  assert.doesNotMatch(result.messages.at(-1).content, /全局后置：阿塔/);
});
...
test('buildWritingPrompt 在写作模式下合并多角色条目、状态与写作专属开关', async () => {
  ...
  assert.match(result.messages[1].content, /写作后置：叙述者/);
  assert.match(result.messages.at(-1).content, /当前场景/);
  assert.match(result.messages[1].content, /next_prompt/i);
  assert.doesNotMatch(result.messages.at(-1).content, /写作后置：叙述者/);
});
