...
test('parseVirtualDate：正确解析 ISO "YYYY-MM-DDTHH:mm" 格式', async () => {
  const { parseVirtualDate } = await freshImport('backend/memory/diary-generator.js');
  const result = parseVirtualDate(JSON.stringify('1000-03-15T14:30'));
  assert.deepEqual(result, { year: 1000, month: 3, day: 15 });
});

test('parseVirtualDate：拒绝非 ISO 格式（旧 N年N月... 不再支持）', async () => {
  const { parseVirtualDate } = await freshImport('backend/memory/diary-generator.js');
  assert.equal(parseVirtualDate(JSON.stringify('1000年3月15日14时30分')), null);
  assert.equal(parseVirtualDate(JSON.stringify('1000-3-15T14:30')), null);
});
