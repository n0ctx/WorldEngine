...
function insertNearby(db, { sessionId, name, persona = '', isSaved = 0 }) {
...
test('applyNearbyResult: ref_id 命中 → 更新 name/persona/state', async () => {
  ...
  applyNearbyResult({
    ...
    fields,
    nearby_characters: [
      { ref_id: id, name: 'Alice2', persona: '新记忆', state: { mood: '兴奋', hp: 75 } },
    ],
  ...
  assert.equal(rows[0].persona, '新记忆');
...
test('applyNearbyResult: ref_id=null + name 命中 → 等同更新', async () => {
  ...
  applyNearbyResult({
    ...
    fields,
    nearby_characters: [
      { ref_id: null, name: 'Bob', persona: '更新后的记忆', state: { mood: '冷静' } },
    ],
  ...
  assert.equal(rows[0].persona, '更新后的记忆');
  const states = listAllStateValues(sandbox.db, id);
...
test('applyNearbyResult: ref_id=null + name 不在池 → 新建 transient', async () => {
  ...
  applyNearbyResult({
    ...
    fields,
    nearby_characters: [
      { ref_id: null, name: 'Carol', persona: '初次登场', state: { mood: '紧张', hp: 60 } },
    ],
    pool: [],
  });
  ...
  assert.equal(rows[0].persona, '初次登场');
...
test('applyNearbyResult: 非法 ref_id 整条丢弃；不影响其他项；不影响 saved', async () => {
  ...
  applyNearbyResult({
    ...
    nearby_characters: [
      ...
      { ref_id: null, name: 'Dora', persona: '正常项', state: { mood: '微笑' } },
    ],
...
test('applyNearbyResult: 池里没回的 transient 删除，saved 保留', async () => {
  ...
  const savedId = insertNearby(sandbox.db, { sessionId, name: 'Keep', persona: '保留 saved', isSaved: 1 });
  ...
  // saved 的 state/persona 不变
  assert.equal(rows[0].persona, '保留 saved');
...
test('applyNearbyResult: 未启用字段被 LLM 写入 → 跳过', async () => {
  ...
  applyNearbyResult({
    ...
    fields: enabledOnly,
    nearby_characters: [
      { ref_id: id, name: 'Eve', persona: 'm', state: { mood: '惊讶', secret: '不该写' } },
    ],
