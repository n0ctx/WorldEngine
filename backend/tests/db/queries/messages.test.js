...
test('getMessagesBySessionId 会在相同 created_at 下保持稳定插入顺序，并让 deleteMessagesAfter 删除同毫秒后续消息', async () => {
  const world = insertWorld(sandbox.db, { name: '消息世界-同毫秒排序' });
  const character = insertCharacter(sandbox.db, world.id, { name: '四号' });
  const session = insertSession(sandbox.db, { character_id: character.id });
  const { createMessage, getMessagesBySessionId, deleteMessagesAfter } = await freshImport('backend/db/queries/messages.js');
  ...
  assert.deepEqual(
    getMessagesBySessionId(session.id, 50, 0).map((row) => row.content),
    ['同毫秒-1', '同毫秒-2', '同毫秒-3'],
  );
