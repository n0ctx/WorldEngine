...
test('写作 generate 与 continue 在选项区未闭合时会用副模型补齐选项', async () => {
  resetMockEnv();
  ctx.sandbox.writeConfig({
    ...ctx.sandbox.readConfig(),
    writing: {
  ...
  const world = insertWorld(ctx.sandbox.db, { name: '写作补齐世界' });
  let res = await ctx.request(`/api/worlds/${world.id}/writing-sessions`, { method: 'POST' });
  const session = await res.json();
  ctx.sandbox.db.prepare('UPDATE sessions SET title = ? WHERE id = ?').run('已有标题', session.id);
  ctx.sandbox.db.prepare(
    'INSERT INTO chapter_titles (id, session_id, chapter_index, title, is_default, created_at, updated_at) VALUES (?, ?, ?, ?, ?, ?, ?)',
  ).run('chapter-existing', session.id, 1, '既有章节标题', 0, 1, 1);
