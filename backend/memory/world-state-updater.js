...
import { getSessionById, getMessagesBySessionId } from '../services/sessions.js';
...
export async function updateWorldState(worldId, sessionId) {
  ...
  const character = session ? getCharacterById(session.character_id) : null;
  ...
  const prompt = [
    {
      role: 'user',
      content:
        `你是世界状态追踪系统，负责根据对话内容更新世界"${world.name}"的状态。\n\n` +
        `候选状态字段：\n${fieldsDesc}\n\n` +
        `最近对话：\n${dialogue}\n\n` +
        `要求：\n` +
