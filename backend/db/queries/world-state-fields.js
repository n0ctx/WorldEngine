...
/**
 * 创建世界状态字段，sort_order 默认取同 world 最大值 + 1
 */
export function createWorldStateField(worldId, data) {
  const id = crypto.randomUUID();
  const now = Date.now();
  const maxRow = db.prepare(
    'SELECT MAX(sort_order) AS m FROM world_state_fields WHERE world_id = ?',
  ).get(worldId);
  ...
  return getWorldStateFieldById(id);
}

/**
 * 根据 id 获取单个字段定义
 */
export function getWorldStateFieldById(id) {
  return parseRow(db.prepare('SELECT * FROM world_state_fields WHERE id = ?').get(id));
}

/**
 * 获取某世界的所有字段定义，按 sort_order ASC
 */
export function getWorldStateFieldsByWorldId(worldId) {
  return parseAll(
    db.prepare(
      'SELECT * FROM world_state_fields WHERE world_id = ? ORDER BY sort_order ASC, created_at ASC',
    ).all(worldId),
  );
}
...
export function updateWorldStateField(id, patch) {
  ...
  for (const field of allowed) {
    ...
    sets.push(`${field} = ?`);
    if (field === 'trigger_keywords' || field === 'enum_options') {
  ...
  if (sets.length === 0) return getWorldStateFieldById(id);

  sets.push('updated_at = ?');
  values.push(Date.now(), id);
  db.prepare(`UPDATE world_state_fields SET ${sets.join(', ')} WHERE id = ?`).run(...values);
  return getWorldStateFieldById(id);
}
...
export function deleteWorldStateField(id) {
  return db.prepare('DELETE FROM world_state_fields WHERE id = ?').run(id);
}

/**
 * 批量重排序：orderedIds[0] 的 sort_order = 0，依次递增
 */
export function reorderWorldStateFields(worldId, orderedIds) {
  const update = db.prepare(
    'UPDATE world_state_fields SET sort_order = ? WHERE id = ? AND world_id = ?',
  );
