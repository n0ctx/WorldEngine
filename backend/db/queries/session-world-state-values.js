...
/**
 * 获取某会话下某世界的所有运行时状态值，返回 { field_key → runtime_value_json } Map
 */
...
/**
 * 清空某会话的所有世界运行时状态（消息回滚时调用）
 */
export function clearSessionWorldStateValues(sessionId) {
  db.prepare('DELETE FROM session_world_state_values WHERE session_id = ?').run(sessionId);
}
