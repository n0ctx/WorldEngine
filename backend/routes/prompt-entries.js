...
router.post('/worlds/:worldId/entries', (req, res) => {
  const { title, description, content, keywords, keyword_scope, position, trigger_type, sort_order, token } = req.body;
  if (!title) return res.status(400).json({ error: 'title is required' });
  const entry = createWorldPromptEntry(req.params.worldId, { title, description, content, keywords, keyword_scope, position, trigger_type, sort_order, token });
