...
export const TABLE_SCHEMAS = {
  ...
  plotlines: { name: '定局表', columns: ['既定事实', '涉及', '红线'],
    defaultMaxRows: 25,
    purpose: '记录「已经发生、且后续绝不能自相矛盾」的关键硬事实，是给主笔的「红线清单」，不是待办任务清单——这里没有「进行中」这种东西，凡记入的都已闭合。**判据：一件事如果主笔忘了就会写出矛盾（穿帮）→ 记；还没发生、只是悬念或未达成的目标 → 不记。**「既定事实」写已发生的硬事实，一句话名词化（如「王被毒杀，凶手是宰相」）；「涉及」写关联角色/地点便于检索；「红线」写这条事实约束了后续什么不能写（如「无人知晓真凶；王不可复活」）。**与历史召回分工：召回是「剧情日记」（记发生过的事、可细、忘了也不穿帮），本表是「红线清单」（只挑会造成穿帮的硬事实、常驻提醒主笔）；同一件事可能两边都有，不算重复，不要为了去重而漏记红线。**只有当剧情回滚或改设定让某条事实不再成立（retcon）时才 close（reason 写「设定撤销」）；正常推进永远不 close 本表的行。' },
