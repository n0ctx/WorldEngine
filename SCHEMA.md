...
每轮对话结束后（状态更新完毕后）创建一条记录，包含该轮的摘要文本和带状态快照的原文。
...
- `search(queryVector, { worldId, currentSessionId, sameSessionThreshold, crossSessionThreshold, topK })`：同 session 内使用宽松阈值（`MEMORY_RECALL_SAME_SESSION_THRESHOLD = 0.45`），跨 session 使用严格阈值（`MEMORY_RECALL_SIMILARITY_THRESHOLD = 0.68`）
