...
- **父子代理稳定 prefix 缓存**：父子代理调 `llm.resolveToolContext` / `llm.chat` / `llm.completeWithTools` 时显式传 `cacheableSystem`（父代理 = `parent-agent.md + CONTRACT.md`，子代理 = `sub-agent.md + 当前 targetType 对应 knowledge.md`）。Anthropic 自动 prefix cache，Gemini 触发 explicit `cachedContents`，其他 provider 忽略。仅为 cache 提示，不影响 prompt 内容。
...
- **取消语义**：父代理工具执行包装层在工具开始前/完成后都会检查 `task.status`；一旦已取消，直接抛专用 `ToolLoopCancelledError`，provider 侧 `resolveToolContext` 立即终止整个 tool-use 循环，不把“已取消”包装成普通 tool result 再返给模型继续下一轮。子代理同步父代理 cancel 状态：`dispatchSubAgent` 接收 `cancelCheck` 闭包（`() => task.status === 'cancelled'`），通过 `adapter.js wrapToolEvents` 在工具执行前/后检查，触发 `ToolLoopCancelledError`；由 `completeAnthropicWithTools` 与 `resolveToolContextAnthropic`（Sprint C Task 1 修复）透传出来，经 `dispatchSubAgent` 外层 catch 转为 `{ success:false, error }`，父代理 `dispatch_subagent.execute` 映射为 `{ ok:false, error }`，确保用户点“清空”后正在落库的子代理立即中断，不会继续执行后续 apply_* 工具。
