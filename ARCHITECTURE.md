...
/backend/
  ...
  /utils/
    ...
    session-summary-vector-store.js  session summary 内存向量索引（T35 起不再写入，存档旧数据）
    turn-summary-vector-store.js     turn record 摘要内存向量索引（T35，双阈值搜索）
...
/data/
  ...
  /vectors/
    ...
    session_summaries.json           Session summary embedding 索引（T35 起不再写入，存档）
    turn_summaries.json              Turn record 摘要 embedding 索引（内存加载，T35）
```
...
MEMORY_RECALL_SAME_SESSION_THRESHOLD = 0.45     # 同 session 内宽松阈值（T35）
...
| `data/vectors/session_summaries.json` | Session summary embedding 索引 | T35 起不再写入，存档旧数据 |
| `data/vectors/turn_summaries.json` | Turn record 摘要 embedding 索引 | 启动时加载到内存（T35） |
