...
- **计划文档**：物理文件落在 `/.temp/assistant/<taskId>.md`（`assistant/server/plan-doc.js` 维护原子读写、状态字段切换），每次 `write_plan_doc` / `edit_plan_doc` 后 emit `plan_doc_updated` SSE，并同步写入 `assistant_tasks.messages_json` 的 `plan_doc` UI 记录，前端 `PlanDocViewer` 渲染。计划文档必须体现真实依赖：读/确认现状 → 定义字段或条目 → 创建/定位目标资源 → 写值/更新正文 → 核对验收。状态值填写按 3-5 个字段一组拆成多个 persona-card / character-card update step，并在 step.task 中列出每组字段的 `field_key`、label、type 与目标 `value_json`，避免单个子代理漏填。
- **计划拒绝语义**：用户在 `awaiting_approval` 点击“拒绝计划”会走 `/agent/:taskId/reject`，不触发 `/cancel`，也不把任务切到 `cancelled`。服务端清掉当前 plan doc 与 `plan_doc` 消息，切到 `paused` 并发 `messages_changed + paused + task_snapshot`；前端输入框保持可用，用户可继续给修改意见或换一个目标。
