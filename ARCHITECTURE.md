...
> 目标读者：AI 助手。覆盖式更新，不追加历史。最后更新：2026-04-16
...
/backend/
  ...
  /utils/
    ...
    session-summary-vector-store.js  session summary 内存向量索引
...
/data/
  ...
  /vectors/
    ...
    session_summaries.json           Session summary embedding 索引（内存加载）
```
...
8 段顺序，[1]–[6] 合并为单条 `role:system`，[7] 为多条 `role:user/assistant`，[8] 为单条 `role:user`：
...
| [pre-6] | `session.compressed_context`，格式：`[早期对话摘要]\n${content}` | NULL 跳过 |
| [4] | `character.system_prompt` | 空跳过 |
| [5] | Prompt 条目（全局→世界→角色顺序；命中→`entry.content`，未命中→`entry.summary`） | 无条目时跳过 |
| [6] | recall.js 渲染结果，顺序：玩家状态→角色状态→世界状态→时间线→召回摘要→展开原文 | 全部为空则整段跳过 |
| [7] | `getUncompressedMessagesBySessionId(sessionId)`；每条 content 经 `applyRules(content, 'prompt_only', worldId)` 处理 | — |
| [8] | 后置提示词（`global_post_prompt` → `world.post_prompt` → `character.post_prompt`），非空部分合并为单条 `role:user` | 均空跳过 |
...
| 段 | 差异 |
|---|---|
| [4] | 无单一角色；从 `writing_session_characters` 获取激活角色列表；每个角色格式：`[角色：${name}]\n${system_prompt}` |
| [5] | 合并全局 + 世界 + 所有激活角色的 entries |
| [6] | 无向量召回（不调用 `searchRecalledSummaries`），无记忆展开；循环所有激活角色调用 `renderCharacterState` |
| [8] | 无角色后置提示词（只有 `global_post_prompt` + `world.post_prompt`） |
...
**regenerate**：先 `clearPending(sessionId, 4)` 清空优先级 ≥4 的待处理任务，再同上入队。

**maybeCompress 内部流程**（按轮数阈值决定是否触发）：

```
maybeCompress(sessionId)
  ├─ 未达阈值：直接返回
  └─ 达阈值：
       1. generateSummary(sessionId)          → summary 文本
       2. setCompressedContext(sessionId, ..)  → 写入 sessions.compressed_context
       3. markAllMessagesCompressed(sessionId) → messages.is_compressed = 1
       4. upsertSessionTimeline(worldId, ..)   → 写入/更新 world_timeline
       5. embedSessionSummary(sessionId)       → 向量化写入 session_summaries.json
```
...
| `searchRecalledSummaries(worldId, sessionId)` → Promise<{recalled, recentMessagesText}> | — | 向量搜索；recalled 数组含 {ref, session_id, session_title, created_at, content, score} |
| `renderRecalledSummaries(recalled)` → string | `[历史记忆召回]` | 格式：`【#ref】【date · title】content` |

**[6] 注入顺序**：玩家状态 → 角色状态 → 世界状态 → 时间线 → 召回摘要 → 展开原文；全部为空则 [6] 为空字符串，整段跳过。

**向量搜索行为**：
- topK = `MEMORY_RECALL_MAX_SESSIONS`（3）
- 相似度阈值 `MEMORY_RECALL_SIMILARITY_THRESHOLD`（0.68）
- token 预算软截断（`MEMORY_RECALL_MAX_TOKENS` = 2048），超额时 break（不包含超额项）
...
| `prompt_only` | assembler.js [7] 历史消息处理时 | 否 | 否 | 是 |
...
| `session` | 删除该会话所有消息的附件文件 + 该会话的 summary embedding |
| `character` | 删除相关消息附件 + 角色头像文件（`avatar_path`）+ prompt entries embedding + 该角色所有会话的 summary embedding |
| `world` | 删除相关消息附件 + 所有角色头像 + persona 头像 + 所有角色 prompt entries embedding + 所有会话 summary embedding |
...
MEMORY_RECALL_SIMILARITY_THRESHOLD = 0.68
...
| `data/vectors/session_summaries.json` | Session summary embedding 索引 | 启动时加载到内存 |
