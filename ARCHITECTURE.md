...
- **暂停语义（spec §6.4）**：`executing` 期间用户消息 enqueue 到 `task.pendingUserMessages`，不打断当前 step；step 跑完后父代理 `dispatch_subagent` 工具内调用 `taskStore.takeUserMessages()`，若有挂起消息则切 `paused`、emit `paused` 并把消息追加到 `task.messages`，随后抛 `ToolLoopControlSignal('paused')` 立即停止后续派发。下一轮用户消息进入 `runParentAgent` 时再由父代理结合更新后的对话历史与 plan-doc 继续规划。
...
- **上下文压缩**：`task.messages` 永远保留完整文本历史供 UI 回放；发给父代理 LLM 前，`parent-agent.js` 会在 `messages.length > 8` 或总字符数超过 `24000` 时，把更早前缀压成一条独立的 `role:system` 历史摘要，并把摘要元数据持久化到 `assistant_tasks.model_context_json`。若用户随后 truncate / delete 历史，导致上下文重新落回阈值内，则旧 `modelContext` 会被立即清空，避免已删除指令继续通过摘要残留。模型输入始终保持“摘要 + 最近最多 8 条原始消息 + 当前 context block”，日志记录 `contextCharsBefore` / `contextCharsAfter` / `summaryUsed` / `tailMessageCount` 供诊断 token 预算。
...
- **任务态持久化**：每次 mutator（createTask / setStatus / appendMessage / updateMessageContent / deleteMessage / truncateFrom / queueUserMessage / takeUserMessages / setModelContext / setCurrentStep / setError）都会把整条任务快照写入 SQLite `assistant_tasks` 表。模块加载时先扫描 `ASSISTANT_STATE_DIR` 下旧 `.json` sidecar 导入 DB，再从 `assistant_tasks` hydrate 到内存 Map。恢复策略：
  ...
  - `completed` / `failed` / `cancelled`：原样恢复。
  - SSE 客户端不持久化。
