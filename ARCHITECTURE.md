...
/frontend/src/
  ...
  components/
    ...
    edit/                            跨页面编辑流支架（EditPageShell、AvatarUpload）
...
- **父子代理稳定 prefix 缓存**：父代理调 `llm.completeWithToolsDetailed`、子代理调 `llm.completeWithTools` 时显式传 `cacheableSystem`（父代理 = `parent-agent.md + CONTRACT.md`，子代理 = `sub-agent.md + 当前 targetType 对应 knowledge.md`）。Anthropic 自动 prefix cache，Gemini 触发 explicit `cachedContents`，其他 provider 忽略。仅为 cache 提示，不影响 prompt 内容。
- **任务规模判定（simple / plan）**：父代理在首轮根据需求拆出步骤计划；步骤数 < 3 走 simple mode，父代理直接调用对应 `apply_*` 落库；步骤数 ≥ 3 走 plan mode，调用 `write_plan_doc` 写计划 → 切 `awaiting_approval` → 等用户 `/approve` → 父代理逐步 `dispatch_subagent`，并按需 `edit_plan_doc` 维护勾选与日志。子代理软失败统一映射为 `{ok:false, error}`，不再使用 `success` 字段，避免父代理 LLM 把失败误判成成功推进。
- **计划文档**：物理文件落在 `/.temp/assistant/<taskId>.md`（`assistant/server/plan-doc.js` 维护原子读写、状态字段切换），每次 `write_plan_doc` / `edit_plan_doc` 后 emit `plan_doc_updated` SSE，并同步写入 `assistant_tasks.messages_json` 的 `plan_doc` UI 记录，前端 `PlanDocViewer` 渲染。
...
- **单通道执行 + 显式控制流短路**：`runParentAgent` 不再走“先 `resolveToolContext`，再 `chat`”双阶段，而是一次调用 `llm.completeWithToolsDetailed(messages, tools)` 完成研究、工具执行和普通文本终稿生成。`write_plan_doc` / `dispatch_subagent(触发 pause)` / `finalize_task` 成功后直接抛 `ToolLoopControlSignal(kind)` 透传出 provider-agnostic tool loop；`runParentAgent` 显式处理三类信号：`awaiting_approval` 保持长连接等待 `/approve`；`paused` 保持长连接等待下一轮用户调整；`terminal` 立即发 `done` 并关闭 SSE，不再存在“工具后再猜要不要进 Step 2”。
...
- **暂停语义（spec §6.4）**：`executing` 期间用户消息 enqueue 到 `task.pendingUserMessages`，不打断当前 step；若最后一个 SSE 订阅者断开，`taskStore.detachSse()` 会设置一次性 `pauseRequested` 标记。step 跑完后父代理 `dispatch_subagent` 工具内同时消费 `taskStore.takeUserMessages()` 与 `consumePauseAfterCurrentStep()`：只要有挂起消息，或存在断连暂停请求，就会切 `paused`、emit `paused`；挂起消息会追加到 `task.messages`，随后抛 `ToolLoopControlSignal('paused')` 立即停止后续派发。下一轮用户消息进入 `runParentAgent` 时再由父代理结合更新后的对话历史与 plan-doc 继续规划。
...
- **任务态持久化**：每次 mutator（createTask / setStatus / appendMessage / updateMessageContent / deleteMessage / truncateFrom / queueUserMessage / takeUserMessages / setModelContext / setCurrentStep / setError）都会把整条任务快照写入 SQLite `assistant_tasks` 表；`taskStore.emit()` 对 `tool_call_*` / `step_*` / `plan_doc_updated` 事件会先同步维护 `messages_json` 中的 UI 记录再广播 SSE，因此刷新页面或重启后仍可回放工具调用记录。模块加载时先扫描 `ASSISTANT_STATE_DIR` 下旧 `.json` sidecar 导入 DB，再从 `assistant_tasks` hydrate 到内存 Map。恢复策略：
  - `awaiting_approval` / `paused`：原样恢复，可继续 `/approve` 或后续对话。
  - `planning` / `executing`：统一转 `failed` + `error='interrupted by restart'`，避免恢复到一个父代理循环已死亡的中间态。
  - `completed` / `failed` / `cancelled`：原样恢复；其中 `completed` 仅作为历史快照回放，不再禁用输入框，用户可直接在该任务上下文里续发新消息。
  - SSE 客户端不持久化。
