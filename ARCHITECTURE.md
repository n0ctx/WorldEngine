...
/data/
  ...
  /logs/
    worldengine-YYYY-MM-DD.log       运行时日志，按日轮换（T99）；data/.gitignore 已覆盖，不提交 git
```
...
- 运行时状态写入统一由 `combined-state-updater.js` 负责；编辑页只改默认值层
...
| `/api/sessions` | routes/chat.js | `/:id/chat` `/:id/stop` `/:id/regenerate` `/:id/continue` `/:id/impersonate` `/:id/edit-assistant` `/:id/summary` `DELETE /:id/messages` |
...
| `/api` | routes/world-timeline.js | `GET /worlds/:id/timeline` 只读 |
...
| `data/logs/worldengine-YYYY-MM-DD.log` | 运行时日志，按日轮换 | T99；`data/.gitignore` 已覆盖，不提交 git |
