...
/assistant/
  ...
  server/                            助手后端（routes / task-researcher / task-planner / task-executor / task-store / agents / tools）
  client/                            助手前端面板
...
- `/api/assistant/chat`、`/api/assistant/tasks`、`/api/assistant/tasks/:taskId/answer|approve-plan|approve-step`、`/api/assistant/extract-characters`
...
**写卡助手两条链路**（同挂 `/api/assistant`）：
- 兼容 chat 轨：`/chat` 维持原有"主代理 + Agent Skill + proposal 卡"模式
- 通用 task 轨：`/tasks` 系列驱动 `Task -> Research -> Plan -> Step DAG -> Proposal -> Apply`，详见 `assistant/CONTRACT.md`

**计划闸门**：简单低风险 create 跳过审批；3 步以上、高风险、已有实体 update/delete、或 research 标记 `needsPlanApproval=true` 进入 `awaiting_plan_approval`。

**子代理重试策略**：JSON 解析失败重试一次；JSON 可解析但 `normalizeProposal` 因 proposal 契约失败时，把具体错误反馈给同一子代理再重试一次（定向修复，不重写无关内容）。

**高风险步骤审阅流**：高风险 step 先生成完整 proposal，推送 `step_proposal_ready` 事件；前端任务面板用 `ChangeProposalCard` 编辑后，`approve-step` 把 `editedProposal` 回传服务端二次 `normalizeProposal` 才允许 apply。

**CUD 术语约束**：planner / 主代理 / 执行子代理在卡片正文、条目内容、状态字段说明、开场白、step task 中统一用 `{{user}}` 指代代入者，`{{char}}` 指代模型扮演角色；schema 字段值与历史状态标签保持原格式（如 `target:"persona"`、`keyword_scope:"user"`、`target_field:"玩家.HP"`）。

**world-card assistant 对齐规则**：
- `preview_card(target="world-card")` 返回 `trigger_type='state'` 条目时附带 `conditions`
- `editedProposal.entryOps[].conditions` 与 `entry_conditions` 表同构：`target_field` 用 `世界.xxx / 玩家.xxx / 角色.xxx`，`operator` 用当前评估器支持的符号/中文操作符
- world-card 提案卡内联编辑器按真实条目编辑模型渲染
...
**辅助工具 `preview_card` 与 `read_file`** 对主代理和执行子代理均可用；proposal schema、task schema、SSE 事件白名单见 `assistant/CONTRACT.md`。
