...
/backend/
  ...
  /llm/
    ...
    embedding.js                     embed()
    /providers/                      openai.js / ollama.js（`kimi-coding` / `minimax-coding` 走 Anthropic-compatible；`glm-coding` 走 OpenAI-compatible）
  /utils/
...
自 T170 起，prompt 采用 **cached/dynamic 分层** 以支持 Anthropic Prompt Cache。

- **Cached layer**：标记为 `cache_control: { type: 'ephemeral' }`，能被 Anthropic API 缓存以加速重复对话
- **Dynamic layer**：每轮变化的上下文，通过 user 角色消息传输
...
13 段顺序（以执行顺序重新编号），Cached 段合并为单条 `role:system` 并标记 cache_control，Dynamic 段为 `role:user`，Historical 为多条 `role:user/assistant`，Bottom 追加到末尾 `role:user`：
...
**Cached layer 的发送方式**：Anthropic provider 会将 system 消息自动包装为 `[{ type: 'text', text: system, cache_control: { type: 'ephemeral' } }]`（见 `withCacheControl` 函数）。
