...
> ⚠️ **不要直接全量阅读此文件。** 先读 `ARCHITECTURE-INDEX.md` 的任务→章节映射表，按需跳读本节。
...
/frontend/src/
  ...
  assets/fonts/                      自托管字体资源；`fonts.css` 中统一声明 `@font-face`，主题只通过 `--we-font-*` token 引用
...
**主题适配边界**：主题只覆盖 `--we-*` token，组件结构 CSS 永远放在核心样式中。新增前端组件或样式时，必须引用已有 token；需要新视觉语义时先在 `tokens.css` 提供默认值，并尽量从基础 token 派生。这样主题会继承新增界面，不需要随每次前端组件改动同步修改。旧主题兼容别名已全部删除，新主题包、用户自定义 CSS、测试夹具和文档示例都应直接使用当前正式 token 名。
...
- **父代理（`assistant/server/parent-agent.js`）**：长生命周期上下文，每轮在首条 system 自动注入 `assistant/knowledge/CONTRACT.md`；工具集 = 3 读（`preview_card` / `list_resources` / `read_file`）+ 4 meta（`write_plan_doc` / `edit_plan_doc` / `delete_plan_doc` / `dispatch_subagent`）+ 1 终态（`reply_to_user`）。**父代理不再持有 `apply_*` 工具**；任何资源新增 / 修改 / 删除必须通过 `dispatch_subagent` 派给子代理执行。每次 user-turn 走一次 `llm.completeWithTools` 原生 tool-calling：provider 内部完成 tool 多轮往返，父代理在外层处理 `TOOL_LOOP_SIGNAL`（`reply_to_user → terminal/paused`、`write_plan_doc → awaiting_approval`、`dispatch_subagent` 在 pending user 消息时 → paused`）。若模型只输出普通文本且文本中声称“已派发/已创建/正在执行”等动作，但本轮没有真实 `dispatch_subagent` / step / 落地资源，父代理会走 softFail，避免把口头动作误记成完成。若子代理真实返回 `success:false`，`dispatch_subagent` 会直接抛出 `PAUSED` 控制信号，父代理立刻走子任务失败暂停，不再等模型把后半句“完成口径”说完。父代理还会识别高风险、跨资源、从零创建核心卡片、状态/条目体系、完整/全套/批量/补全/整体优化等需要计划的任务，在 context block 注入强制编排提示；若模型跳过计划直接 `dispatch_subagent`，meta 工具会返回错误，要求先 `write_plan_doc`。计划必须至少 3 个可执行 step；用户批准计划后的续跑阶段会拒绝再次 `write_plan_doc`，避免二次确认。
- **通用子代理（`assistant/server/sub-agent.js`）**：每个 step 起一个干净上下文，按 `task.targetType` 注入 `assistant/knowledge/<TARGET>.md`（一次只一份，互不串味）；工具集 = 3 读 + 1 个对应 targetType 的 `apply_*` 工具。子代理直接执行 `normalizeProposal` + 资源落库，并通过 `onApplied` 回调把落地资源（`{ kind, op, stepId, name, refId }`）写入父任务的 `appliedResources` 列表。若本轮 apply 成功次数为 0，则子代理返回失败摘要，`dispatch_subagent` 会即时切换为暂停控制信号，避免父代理继续产出误导性的“已完成”收尾文本。
...
- **计划文档**：物理文件落在 `/.temp/assistant/<taskId>.md`（`assistant/server/plan-doc.js` 维护原子读写、状态字段切换），每次 `write_plan_doc` / `edit_plan_doc` 后 emit `plan_doc_updated` SSE，并同步写入 `assistant_tasks.messages_json` 的 `plan_doc` UI 记录，前端 `PlanDocViewer` 渲染。计划文档必须至少包含 3 个可执行 step，并体现真实依赖：读/确认现状 → 定义字段或条目 → 创建/定位目标资源 → 写值/更新正文 → 核对验收；文档不再保留空的“执行日志”段。只能拆成 1-2 个动作的任务不生成计划，直接派发子代理执行。状态值填写按 3-5 个字段一组拆成多个 persona-card / character-card update step，并在 step.task 中列出每组字段的 `field_key`、label、type 与目标 `value_json`，避免单个子代理漏填。
...
**计划闸门**：普通单资源小改可直接 dispatch；创建世界卡 / 玩家卡 / 角色卡、状态/Prompt 条目体系、批量填写 / 补全 / 初始化状态字段、高风险删除、复杂跨资源修改、完整/全套/从零/整体优化类请求，只有能拆出至少 3 个真实可执行 step 时才写计划文档并切 `awaiting_approval`，由用户 `/approve` 续派。只能拆成 1-2 个动作的任务不生成 plan，直接派发子代理执行；少于 3 个 step 的 `write_plan_doc` 会被工具层拒绝。
