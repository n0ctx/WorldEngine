...
/frontend/src/
  ...
  /components/
    ...
    /ui/                             Button / Input / Textarea / Select / Card / Badge / ModalShell / MarkdownEditor（懒加载包装层）/ MarkdownEditorInner（Tiptap 实现）/ ModelCombobox / FieldLabel / FormGroup / EditPageShell / ConfirmModal / AvatarUpload
    ...
    /state/                          StateFieldEditor / StateFieldList / StateValueField / EntrySection / EntryEditor / TriggerCard / TriggerEditor
  ...
  /pages/
    ...
    WorldStatePage.jsx               世界状态页（State 条目 + 触发器管理）
  /utils/
...
| `trigger_fired` | 状态更新后命中触发器且产生通知 | `{ type: "trigger_fired", notifications: [{ name: "触发器名", text: "..." }] }` |
...
| 方法 | 路径 | 说明 |
|---|---|---|
| GET | /api/worlds/:worldId/triggers | 列出世界全部触发器（含 conditions/actions） |
| POST | /api/worlds/:worldId/triggers | 创建触发器（body: name/enabled/one_shot/conditions/actions） |
| PUT | /api/triggers/:id | 更新触发器（可整体替换 conditions/actions） |
