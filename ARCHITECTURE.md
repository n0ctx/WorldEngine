...
- **取消语义**：父代理工具执行包装层在工具开始前/完成后都会检查 `task.status`；一旦已取消，直接抛专用 `ToolLoopCancelledError`，provider 侧 `resolveToolContext` 立即终止整个 tool-use 循环，不把“已取消”包装成普通 tool result 再返给模型继续下一轮。
