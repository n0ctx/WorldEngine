...
/shared/chapter-constants.mjs  # 前后端共享常量（章节分组阈值），修改须同步
/assistant/knowledge/           # 7 份知识文件：CONTRACT(契约) + WORLDCARD/CHARCARD/USERCARD/GLOBALPROMPT/CSSSNIPPET/REGEXRULE
/assistant/server/              # 写卡助手后端：routes.js / parent-agent.js / sub-agent.js / plan-doc.js / normalize-proposal.js / task-store.js / tools/
/assistant/client/              # 写卡助手前端面板（AssistantPanel、PlanDocViewer 等）
```
...
- Page 层只做组合与数据绑定，禁止写 CSS、内联 style 或新建视觉组件
- 整体 Claude 风格（参考 `DESIGN.md`）：羊皮纸底色 + 陶土强调 + 暖色中性；衬线做标题、无衬线做 UI；环形阴影（`0 0 0 1px`）代替传统投影
- 语言：简体中文
...
- 组件分层：Primitive（视觉原子）→ Block（结构模式）→ Feature（业务语义）→ Page（组合）；Feature 禁止重复实现 Primitive 视觉，Page 禁止新建视觉组件
...
**验证要求**
- 后端改动给出接口/流程级验证方法；前端改动给出页面路径和人工验证步骤；文档改动说明检查了哪些漂移点；无法运行测试时明确说明
