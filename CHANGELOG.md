...
- fix(assistant): `PlanTaskHud` 显示优化：① 任务文本剥去 `**step-n**` 前缀（正则 `/^\*{0,2}step-\d+\*{0,2}\s*/i`），只展示正文；② 当前执行项的 ▶ 三角标替换为 `we-hud-spinner` 转圈动画（复用 `we-tool-spin` keyframe，主色 `var(--we-vermilion)`）；③ 行对齐从 `items-start` 改为 `items-center`，与 spinner 等高对齐。
...
- style(chat,writing): 右栏世界状态块与下方 tab 区之间补一条章节分隔线。`.we-state-panel / .we-cast-panel` 的 `.we-cast-scroll > .we-panel-tab-body:first-child` 加 `padding-bottom: 14px; margin-bottom: 12px; border-bottom: 1px solid var(--we-paper-shadow)`,与 flush header 那条更淡的线形成"双线收尾"的章节边界,避免世界字段直接顶到 tab 行。
...
- style(chat,writing): 已保存的附近角色 tab 标签朱砂加粗。`NearbyPanel.jsx` 的 `perCharSections` 在 `is_saved===1` 时把 label 包成 `<span class="we-section-tab-label--saved">`,pages.css 新增 `.we-section-tab-label--saved { color: var(--we-vermilion); font-weight: 600; }`(active 状态保持朱砂)。
