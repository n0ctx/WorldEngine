...
test('wrapToolEvents: afterCompleted 在成功 / 失败两条路径都被调用，含 success 与 error', async () => {
  ...
  // 失败两条路径都带 failureKind：工具未声明时默认 'runtime'，throw 也归 'runtime'
  assert.deepEqual(calls[1], { success: false, error: 'boom', name: 'bad', failureKind: 'runtime' });
  assert.deepEqual(calls[2], { success: false, error: 'explode', name: 'throws', failureKind: 'runtime' });
});

test('wrapToolEvents: afterCompleted 在 precheck 失败时透传 failureKind', async () => {
  const calls = [];
  const tool = {
    type: 'function',
    function: { name: 'precheck-fail' },
    execute: async () => ({ success: false, error: '参数缺失', failureKind: 'precheck' }),
  };
  await wrapToolEvents(tool, () => {}, { afterCompleted: (info) => calls.push(info) }).execute({});
  assert.deepEqual(calls[0], { success: false, error: '参数缺失', name: 'precheck-fail', failureKind: 'precheck' });
});
