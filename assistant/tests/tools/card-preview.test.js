...
test('createPreviewCardTool 会返回实体详情、现有条目与状态字段', async () => {
  ...
  insertWorldEntry(sandbox.db, world.id, { title: '世界条目', content: '世界内容' });
  insertCharacterEntry(sandbox.db, character.id, { title: '角色条目', content: '角色内容' });
  ...
  assert.equal(characterData.existingEntries.length, 1);
...
test('createPreviewCardTool 会返回 persona-card 与 global-prompt 的完整上下文', async () => {
  ...
  insertGlobalEntry(sandbox.db, { title: '全局条目', content: '全局内容', mode: 'chat' });
  ...
  assert.equal(globalData.existingEntries.length, 1);
  assert.equal(globalData.existingEntries[0].title, '全局条目');
});
