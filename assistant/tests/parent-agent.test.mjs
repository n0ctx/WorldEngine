...
test('dispatch_subagent: 已 applied 过 create 时拒绝重复', async () => {
  ...
  // 新行为：子代理失败不再抛 PAUSED 信号，而是正常返回 success:false，把决策交给父代理 LLM
...
test('write_plan_doc / edit_plan_doc: 参数级失败均标 failureKind=precheck', async () => {
  ...
  const r1 = await writePlan.execute({
    title: '单步计划',
    intent: '只做一件事',
    steps: [{ title: '建世界', targetType: 'world-card', operation: 'create', task: '创建空世界卡' }],
  });
  assert.equal(r1.success, false);
  assert.equal(r1.failureKind, 'precheck');

  const r2 = await editPlan.execute({ op: 'mark_done' });
  assert.equal(r2.success, false);
  assert.equal(r2.failureKind, 'precheck');

  const r3 = await editPlan.execute({ op: 'replace_steps', steps: [{}] });
  assert.equal(r3.success, false);
  assert.equal(r3.failureKind, 'precheck');

  const r4 = await editPlan.execute({ op: 'unknown_op' });
...
test('dispatch_subagent: 失败返回 failureKind=precheck，让父代理区分轻重', async () => {
  ...
  assert.equal(badOp.failureKind, 'precheck');
  // create 带 entityRef
  const createWithRef = await dispatch.execute({ targetType: 'world-card', operation: 'create', entityRef: 'w-exists', task: '创建一个新世界卡' });
  assert.equal(createWithRef.success, false);
  assert.equal(createWithRef.failureKind, 'precheck');
});
...
test('runParentAgent: 截图场景复现——dispatch_subagent task 截断 3 次不应触发熔断（precheck 阈值 8）', async () => {
  ...
  await runParentAgent(task, '试一下');
  assert.equal(task.status, 'paused');
  // 关键断言：不是熔断暂停（截图里的 dispatch_subagent 连续失败 3 次提示文案的前置原因）
  assert.notEqual(task.error, __testables.CONSECUTIVE_TOOL_FAILURES_PAUSE_REASON);
  // 落到"空回复"恢复路径（因为 mock 跑完 3 次工具调用后返回 ''）
  assert.equal(task.error, 'harness recoverable pause');
  clearMockEnv();
});
...
test('runParentAgent: precheck 类失败达到阈值 8 时仍会熔断', async () => {
  ...
  const events = [];
  taskStore.attachSse(task.id, { write: (line) => events.push(line) });
  await runParentAgent(task, '反复截断');
  assert.equal(task.status, 'paused');
  assert.equal(task.error, __testables.CONSECUTIVE_TOOL_FAILURES_PAUSE_REASON);
  clearMockEnv();
});

test('runParentAgent: 连续 runtime 失败 3 次仍会熔断（pause reason 持久化）', async () => {
  // 用 dispatch_subagent 制造 runtime 失败：父代理 mock 派发 dispatch_subagent，子代理走 mock LLM 跑工具循环
  // 但 toolCalls 中没有 apply_world_card 项（这些都是父代理工具名），子代理 applySuccessCount 始终为 0，
  // dispatchSubAgent 返回 { success:false, error: '子代理未成功调用 apply_world_card ...' }，
  // 该 outcome 未带 failureKind → 默认 runtime；连续 3 次触发 runtime 阈值熔断。
  process.env.MOCK_LLM_TOOL_CALLS = JSON.stringify([
    { name: 'dispatch_subagent', arguments: { targetType: 'world-card', operation: 'create', task: '创建一个完整的世界卡 Alice，包含基础字段。' } },
    { name: 'dispatch_subagent', arguments: { targetType: 'world-card', operation: 'create', task: '创建一个完整的世界卡 Bob，包含基础字段。' } },
    { name: 'dispatch_subagent', arguments: { targetType: 'world-card', operation: 'create', task: '创建一个完整的世界卡 Carol,包含基础字段。' } },
  ]);
  process.env.MOCK_LLM_COMPLETE = '';
  const task = taskStore.createTask({ context: {} });
  const events = [];
