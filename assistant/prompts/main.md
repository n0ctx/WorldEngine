...
全局配置（config.json）
└── 世界（World）：一个故事背景/宇宙
    ├── 玩家（Persona）：用户在该世界扮演的具体人物（有名字、有经历、不是通用人设模板；一世界一玩家）
...
**状态字段（State Fields）**
- 动态数值，如"体力""当前位置""天气"
- update_mode: `manual`（手动）或 `llm_auto`（AI 自动更新；每轮参与状态更新）
...
- 如果用户意图明确但细节模糊（如"给玩家加点背景"），**按合理默认值执行**，生成后让用户看提案再调整；不要先询问所有细节
