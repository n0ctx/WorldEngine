...
- 数值：`>` `<` `=` `>=` `<=` `!=`
- 文本：`包含` `等于` `不包含`
- datetime：使用数值操作符（`>` `<` `=` `>=` `<=` `!=`），`value` 必须写完整 ISO 局部时间 `"YYYY-MM-DDTHH:mm"`（如 `"1000-03-15T14:30"` 或 `"238-04-20T00:00"`），按段位解析为整数后逐段比较
...
- `"list"`
- `"boolean"`
- `"datetime"`（ISO 局部时间 `"YYYY-MM-DDTHH:mm"`，年份为正整数、可任意位数；月/日/时/分各 2 位，例 `"1000-03-15T14:30"` 或 `"238-04-20T00:00"`；可选 `prefix` 字段写展示前缀，如 `"第三纪元 "`）
...
- `list` → `"[]"`（空数组；如需预设值：`"[\"线索A\"]"`）
- `boolean` → `"false"`
- `datetime` → `"\"1000-03-15T14:30\""`（ISO 局部时间；年份为正整数、可任意位数；月/日/时/分固定 2 位，禁止其他格式）
