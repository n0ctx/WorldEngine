...
import { createSession, deleteMessage as deleteMessageApi } from '../../../core/api/sessions.js';
...
// chat 页流式收发 / 中断 / 续写 / 选项 / 断点恢复 + 会话切换运行时。
// 与 session-view（currentSession / messageListKey / pendingDiaryInject）耦合不可分，故一并收口。
// 入参：character（新建会话用）、messageListRef / inputBoxRef、currentSessionId + setCurrentSessionId（store 会话身份）、memory（记忆指示器 hook 返回值）。
export function useChatStream({ character, messageListRef, inputBoxRef, currentSessionId, setCurrentSessionId, memory }) {
  ...
  }, [clearMemoryState, clearOptionsState, invalidateCurrentRun, resetContinuationState, setCurrentSessionId]);
