...
describe('persona state values api', () => {
  ...
  it('会按玩家路径读取、更新和重置状态值', async () => {
    ...
    expect(fetch).toHaveBeenNthCalledWith(1, '/api/worlds/world-1/persona-state-values', expect.any(Object));
  ...
  it('失败时会透传 body.error', async () => {
    fetch.mockResolvedValueOnce({ ok: false, status: 500, json: async () => ({ error: 'persona state failed' }) });
    await expect(getPersonaStateValues('world-2')).rejects.toThrow('persona state failed');
  });

  it('按 personaId 读取 / 更新状态值并在失败时透传 body.error', async () => {
    fetch
      .mockResolvedValueOnce({ ok: true, json: async () => ([{ field_key: 'mood' }]) })
      .mockResolvedValueOnce({ ok: true, json: async () => ({ ok: true }) });
    ...
    expect(fetch).toHaveBeenNthCalledWith(1, '/api/worlds/w1/personas/p1/state-values', expect.any(Object));
    ...
    fetch.mockResolvedValueOnce({ ok: false, status: 404, json: async () => ({ error: 'no persona' }) });
    await expect(getPersonaStateValuesByPersonaId('w1', 'p2')).rejects.toThrow('no persona');

    fetch.mockResolvedValueOnce({ ok: false, status: 400, json: async () => ({ error: 'patch fail' }) });
  ...
  it('resetPersonaStateValuesByPersonaId 失败时优先 body.error，否则回退状态码', async () => {
    ...
    expect(fetch).toHaveBeenLastCalledWith('/api/worlds/w1/personas/p1/state-values/reset', expect.objectContaining({ method: 'POST' }));
